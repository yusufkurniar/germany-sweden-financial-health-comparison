...
```{r prepare-IDA}
# 1) Long data with *original* variable names kept
...
# 2) Split using original names (no label-matching issues)
...
# 3) Safe label mapping (unknown names keep themselves)
nice <- c(
...
```{r IDA-scale}
# 4) SCALE variables (log axis). Do NOT drop rows entirely—only make y NA if <=0
p_scale <- ida_germany %>%
  filter(variable_orig %in% scale_vars) %>%
  mutate(y = if_else(value > 0, value, NA_real_)) %>%  # keep rows, drop non-positives from y only
