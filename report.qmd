...
filtered_germany_1 <- filtered_germany_1 |>
  mutate(
    ...
    period = factor(period,
      levels = c("Pre-pandemic (2018–2019)", "Pandemic (2020–2021)"),
      labels = c("Pre (2018–19)", "Pandemic (2020–21)")
    )
  )

#helper for IQR error bar
iqr_bar <- function(y){
...
#facet chart (all metrics)

p_q1_all <- ggplot(plot_long, aes(x = period, y = value)) +
  geom_violin(trim = FALSE, fill = NA, linewidth = 0.45) +
  geom_jitter(width = 0.08, height = 0, alpha = 0.10, size = 0.55) +
  stat_summary(fun.data = iqr_bar, geom = "errorbar", width = 0.12, linewidth = 0.5) +
  stat_summary(fun = median, geom = "point", shape = 95, size = 8) +
  facet_wrap(~ metric, scales = "free_y", ncol = 2) +
  scale_x_discrete(drop = FALSE) +
  labs(x = NULL, y = NULL,
       title = "Pre vs Pandemic: Profitability Distributions by Metric (Median + IQR)") +
  theme_minimal(base_size = 11) +
  theme(
    strip.text  = element_text(face = "bold"),
...
The chart compares profitability before (2018–19) and during the pandemic (2020–21) for all firms.
Across EBIT and EBITDA margins, the medians remain largely unchanged, but the spreads widen in 2020–21, indicating increased dispersion in operating performance. ROA and especially ROE show many more extreme negatives in 2020–21, indicating that a greater share of firms reported losses or equity write-downs, even though the median stayed near zero. Net income (asinh) also becomes more dispersed in 2020–21, with a heavier lower tail, confirming higher volatility. A key insight is that central tendencies barely moved while tail risk increased.
